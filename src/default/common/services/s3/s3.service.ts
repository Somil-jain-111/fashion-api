...
export class S3Service {
  ...
  private parseBase64File(
    ...
    if (!mimeType) {
      throw new BadRequestException(
        "mimeType is required when base64 does not contain data URI",
      );
    }
  ...
  private async validateAllChunksExist(
    uploadDir: string,
    totalChunks: number,
  ): Promise<void> {
